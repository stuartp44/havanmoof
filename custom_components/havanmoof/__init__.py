...
def process_discovered_service_info_in_executor(hass):
    ...
    try:
        ...
        for service_info in service_infos:
            # Check if any of the service UUIDs are in the VanMoof UUIDs set
            if any(uuid in VANMOOF_UUIDS for uuid in service_info.service_uuids):
                _LOGGER.info("Discovered VanMoof service: UUIDs=%s, Address=%s, RSSI=%s",
                             service_info.service_uuids, service_info.address, service_info.rssi)
    except Exception as e:
        _LOGGER.error("Error processing discovered service info: %s", e)